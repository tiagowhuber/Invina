        ...
        <CardContent>
          <div v-if="toursStore.tours.length > 0" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="text-center p-4 bg-green-50 rounded-lg">
              <p class="text-2xl font-bold text-green-700">{{ toursStore.tours.filter(t => t.isActive).length }}</p>
              <p class="text-sm text-gray-600">Tours Activos</p>
            </div>
            <div class="text-center p-4 bg-gray-50 rounded-lg">
              <p class="text-2xl font-bold text-gray-700">{{ toursStore.tours.filter(t => !t.isActive).length }}</p>
          ...
          <div v-if="toursStore.tours.length > 0" class="space-y-4">
            ...
            >
              <div class="flex justify-between items-start mb-4">
                <div>
                  <h3 class="font-semibold text-lg">{{ tour.description }}</h3>
                  <p class="text-sm text-gray-600">{{ formatPrice(tour.basePrice) }} por persona</p>
                </div>
                <span :class="tour.isActive ? 'px-2 py-1 bg-green-100 text-green-800 text-xs rounded' : 'px-2 py-1 bg-gray-100 text-gray-800 text-xs rounded'">
              ...
              <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div>
                  ...
                  <p class="font-medium">{{ tour.tourType }}</p>
                </div>
                <div>
                  <p class="text-gray-600">Duración:</p>
                  <p class="font-medium">{{ tour.durationMinutes }} min</p>
                </div>
                <div>
                  <p class="text-gray-600">Capacidad Máx:</p>
                  <p class="font-medium">{{ tour.maxAttendants }}</p>
                </div>
                <div>
                  <p class="text-gray-600">Asistentes Mín:</p>
                  <p class="font-medium">{{ tour.minAttendants }}</p>
                </div>
              </div>
            </div>
          </div>
